...
export async function onBeforeRender(pageContext) {
  ...
  // デフォルトでは、`+onBeforeRender.js` ファイルはサーバーサイドでのみロードおよび実行されます。
  // つまり、ここでSQL/ORMクエリを使用できます。
  const response = await fetch(`https://swapi.dev/api/films/${movieId}`);
  let movie = await response.json();

  // 以前に定義したレンダリングおよびハイドレーション関数は `pageContext.pageProps` を
  // ルートReactコンポーネント `Page` に渡します。ここで `pageProps` を定義します。
  const pageProps = { movie };

  // 最終的に `pageProps` を `pageContext.pageProps` として返却します。
  return {
    pageContext: {
      pageProps
    }
...
// デフォルトでは、pageContextの値はサーバー上でのみ利用可能ですが、先に定義したonRenderClient()
// フックはブラウザで実行され、pageContext.pagePropsの値が必要です。
// これらの値をシリアライズし、ブラウザで利用可能にするためにpassToClientを使用します。
export default {
